...
**Scope cut: no dedicated Reserve sidebar bar or custom end-of-match scoreboard widget yet.** Backlog issue #8 also asked for a persistent per-player Reserve bar and a Reserve-totals scoreboard. Those need Chrome/Lua widget work that has to be iterated against a running, rendered game client, which wasn't available in the environment this phase was implemented in. What *is* shipped from issue #8: the Lockdown start/cancel broadcast (system text line + `Speech` cue, unconditionally visible/audible to every player, not just the affected one) and the minimap reveal above. The sidebar bar and scoreboard remain open under issue #8 as a follow-up, and should be validated visually before merging rather than guessed at blind.
