...
15. **Buildings are `Mesh` solids at `BUILDING_YAW` (45°), on a diamond plinth that fits the footprint (issue
    ...
    Since issue #108 every building also plants its own plot with the greenery vocabulary beside `plinth()`
    (`bed`, `shrub`, `tuft`, `tree`, `vines`, `green_roof`) on palette-exact foliage tones (`LEAF_*`,
    `temperat.pal` 145–152, grey-green so it is plausible on desert, temperate and snow alike); every position
    comes from `_scatter()`, a pure integer hash, so regeneration stays byte-identical. Two rules from that
