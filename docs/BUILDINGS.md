...
- **Prerequisites:** Any power source, Consortium only (`anypower, ~structures.allies, ~techlevel.medium` — the faction gate added in issue #101).
...
- **Staging:** Implemented (Phase 5) as `SGSHL`. Pulled forward from "post-Phase 5" for the same reason as Smart Grid Relay above. **Art (issue #34):** dedicated sprite -- a hardened green dome with a sandbag row along the front edge (per `docs/ART_DIRECTION.md`'s "not utopian" guardrail), replacing the reused `IRON` (Iron Curtain) sprite. **Revised (issue #77, tech-tree audit follow-up):** prerequisites were `fact, dome` — and `fact` is a no-op, since a player without a Construction Yard cannot build anything at all. Replaced with the `techcenter` gate this entry has always described. **Revised (issue #103):** `SpyInfiltrate` **removed** from its `Targetable.TargetTypes`. It was the third building carrying that target type with no `InfiltrateFor*` trait behind it, but unlike `SGCRY` and `SGDAI` there was nothing sensible to give it: it trickles no cash and holds no support power, and its only asset is the `resilience-shelter` proximity aura, which no stock trait can suppress on infiltration. A bespoke `InfiltrateForCondition` trait in `OpenRA.Mods.Sungrid` would have done it and was rejected against `docs/ARCHITECTURE.md`'s data-driven-first rule for a single building. A Spy now correctly declines the target instead of walking in and dying for nothing; if the Shelter later warrants a real infiltration effect, that C# trait is the way to add it.
...
- **Gameplay purpose:** An early, cheap, individually fragile alternative/supplement to Solar Array — rewards spreading power generation out across a base rather than concentrating it, so raiding and map pressure stay relevant on the power layer. Since issue #100 the thing that distinguishes it is **space**: one cell against Solar Array's four, so it slots into gaps a power plant can't use.
...
- **Staging:** Implemented as `SGWND`, part of the energy rebalance pass — see `docs/ENERGY_BALANCE.md`. **Art (issue #34):** treated as an "easy adapt" of Solar Array's own dedicated art rather than a wholly new motif -- two sparse turbine-and-rotor poles on the same ground-strip/gold-conduit visual language `sgpwr.png` established, replacing the reused `SAM` Site sprite it previously shared with both the real `SAM` and `SGTUR`. **Revised (issue #100):** the footprint went 2×2 → **1×1** (`Building: Footprint: x`, `Dimensions: 1,1`, `^1x1Shape`, minibib), and the sprite was redrawn to match — the old 66×54 sheet drew *two* turbines on a shared plinth, sized to the four cells it used to hold, so it read (and cost space) like a small solar farm. It is now a single 40×36 mast on a single-cell footing, in the 1×1 art family `SGREL`/`SGSNS`/`ARCT` already use; the "Array" is the several turbines a player plants around the base. Cost and output are unchanged at 250/+70, which makes this a real buff — the fragility, health-scaled output and blackout exposure that were already its price stay exactly as they were.
...
| SGWND | Wind Turbine Array | Assembly | low | 250 | 30000 | Wood | +70 | anypower | 3.6 cr/power; cheapest power building in the game (was 400 — see gap G3, since retuned). **1×1 since issue #100** — 70 power/cell, the densest in the game |
| SGHYD | Hydrogen Plant | Consortium | high | 1200 | 90000 | Heavy | +350 | dome, atek | 3.4 cr/power; only Heavy-armored power source |
| SGREL | Smart Grid Relay | **Consortium** | med | 400 | 50000 | Heavy | +60 | anypower | 6.7 cr/power; 1×1 footprint, Heavy, vision 7 (was 600 — see gap G4, since retuned). Also **provides** `anypower` since T5. **Consortium-only since issue #101** — was Both, which left the Assembly holding two 1×1 power buildings and the Consortium one |
...
| SGHAU | Hauler Drone | Both | inf | 500 | 22000 | Light | 128 | — | rcyd | Speed was unset (=1, effectively immobile) when first surveyed — see gap G1, since fixed. Later raised 72→100 (matched to HARV, then re-tuned — issue #86) since Light armor/22000 HP couldn't outrun what HARV's Heavy/60000 HP could, then 100→128 (APC parity, still under JEEP 164) when that wasn't enough — issue #97, which also gave it HARV's `ChangesHealth` self-repair |
