...
- Tags are named `alphaN` (`alpha1` … `alpha26` so far) and are cut off `main` whenever there's something worth putting in front of a player — in practice far more often than once per phase.
- **Cutting a release starts with creating the GitHub Release, not with pushing a tag.** `packaging.yml` runs `gh release upload <tag> …` in all three jobs, which requires the Release object to already exist — a bare `git push --tags` fires the workflow and then fails every upload step with "release not found", leaving a tag with no downloads attached. The working order is: **Releases → Draft a new release → new tag `alphaN` targeting `main` → Generate release notes → Publish.** Publishing creates the tag, the tag push triggers `.github/workflows/packaging.yml`, and the workflow attaches Linux AppImages, Windows installers (x86/x64, installer and portable) and a macOS disk image to the release you just published. `alpha25`'s six assets show the pattern: release authored by the maintainer, all six assets uploaded by `github-actions[bot]` three to sixteen minutes later. These are genuinely public downloads, not private tracking tags — check the packaging run actually went green on all three platforms before treating a release as shipped (it silently didn't, on one platform or another, for most of this project's history — see `docs/BACKLOG.md` issues #21, #23, #29, #30).
