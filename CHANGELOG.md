...
- Renamed the two playable sides: **Allies → The Consortium**, **Soviet → The Assembly** — per `docs/BACKLOG.md` issue #15. Every player-facing string updated (the faction picker, Tech Center/Barracks names, tank flavor text); the five real-world-named sub-factions underneath each side (England/France/Germany, Russia/Ukraine) and their special units are unchanged for now — a separate, bigger naming decision tracked in the same issue.
- First-boot menu/intro pass, per `docs/BACKLOG.md` issue #16: fixed a regression where the main-menu shellmap still placed the removed Flame Tower (`ftur`) in six spots (would have broken the main menu background entirely); replaced the window/taskbar/mod-chooser icon (a literal, unmodified stock Soviet star and hammer-and-sickle) with the Phase 6 emblem; reworded two off-theme loading-screen tips. Main menu widget layout, cursors, terrain, the shellmap's own terrain, and the main menu's music are still stock — tracked as open follow-up in the same issue, blocked on either a built engine or Phase 7's audio pass.
