...
- **Red Alert game content.** Sungrid Protocol ships no Red Alert asset files — it reads them from `<SupportDir>/Content/ra/v2/`. On a normal desktop the game offers to install them on first launch (official freeware package, or your own disc/Steam/Origin copy). For a headless/container setup where that dialog can't be used, see the "No game content assets" bullet under "Headless / automated testing" below.
...
- **Grid Reserve** (the economic victory mode) is a per-lobby toggle in the Skirmish setup screen, **checked by default**. Destruction victory (eliminate all opponents) is always available regardless of the toggle — turning Grid Reserve off removes the economic win path, it never removes elimination. A one-time popup explains the rules at the start of a match with the mode on. See [`docs/GAME_MODES.md`](GAME_MODES.md) for the full mechanic.
- **Bot personalities:** six are selectable. **Grid Broker AI** plays the economic mode aggressively; **Easy Grid Broker AI** is the beginner opponent. The four stock personalities (Rush/Normal/Turtle/Naval) also build Vaults now, just less deliberately. See `docs/BACKLOG.md` issues #67/#69.
...
- **Engine fetch when `fetch-engine.sh`'s zip download is blocked:** shallow-clone the exact pinned commit instead. Note the pinned commit lives in **this repo's own history**, not `OpenRA/OpenRA` — `mod.config`'s `AUTOMATIC_ENGINE_SOURCE` points at `richardkfm/sungrid-protocol` (see `CLAUDE.md`'s "Engine version pinning"), so clone from there: `git init && git remote add origin https://github.com/richardkfm/sungrid-protocol.git && git fetch --depth 1 origin <ENGINE_VERSION-sha> && git checkout FETCH_HEAD` into `./engine`, then `rm -rf engine/.git` and write the version marker `fetch-engine.sh` checks for: `echo "<sha>" > engine/VERSION`. `make` will then see the version already matches and skip its own download.
